...
#  To perform a previous seurat analysis of electroporation data, load the provided objects
# and extract counts for cells in each condition
#  Choose which condition you want to analyse (DMSO/EPZ) and comment out the other one

sce_hvg.seurat_DMSO_new <- readRDS("sce_hvg.seurat_DMSO_new.RData")
sce_hvg_1 <- subset(sce_hvg.seurat_DMSO_new, subset= condition=="DMSO")

sce_hvg.seurat_EPZ_new <- readRDS("sce_hvg.seurat_EPZ_new.RData")
sce_hvg_1 <- subset(sce_hvg.seurat_EPZ_new, subset= condition=="EPZ")


#  To perform current seurat analysis of electroporation data, run the markdown file: Code_for_Electroporation_Dataset.Rmd
# load the sce_hvg.seurat object and extract counts for cells in each condition

sce_hvg.seurat <- readRDS("sce_hvg.seurat.RData")

#Choose the condition you want to analyse and comment the other one out
sce_hvg_1 <- subset(sce_hvg.seurat, subset= condition=="DMSO")
sce_hvg_1 <- subset(sce_hvg.seurat, subset= condition=="EPZ")
...
# Plot and save Figure 5A (Left if you run the DMSO subset, Right if you run the EPZ subset)
pheatmap::pheatmap(regulonActivity_byCluster_Scaled, #fontsize_row=3, 
                   ...
                   treeheight_row=10, treeheight_col=10, border_color=NA, filename="regulonActivity_byCellType.pdf", width=10, height=20)
