---
title: "Analysis of Electroporated Cells (E14.5) with Seurat (version 3.1.0) Workflow for scRNA-seq data"
...
```{r}
DimPlot(sce_hvg.seurat, reduction = "umap", pt.size = 2, cols = t)
```

# 19. Plot Figure 4G, Appiah et al. | Note: Order of labels may be different from final figure in manuscript
```{r}
DimPlot(sce_hvg.seurat, reduction = "umap", split.by = "condition", pt.size = 2, cols = t)
```
